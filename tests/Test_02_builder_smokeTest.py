...
from Framework.infrastructure.builder import ResourceFields, construct_building, demolish_building_at, find_building, \
    get_village_data, level_up_building_at
...
def demolish_everything(sws : SWS):
    ...
    ret = False
    ignored_buildings = [BuildingType.EmptyPlace, *ResourceFields, BuildingType.MainBuilding, BuildingType.Wall]
    for site in [siteId for bd, lst in get_village_data(sws).items() if bd not in ignored_buildings and lst for \
            (siteId, _) in lst]:
        if not demolish_building_at(sws, site):
            break
    else:
        ret = True
    return ret
...
def check_buildings_to_be(sws : SWS, expected : list):
    ...
    expected = list(set([*expected, BuildingType.EmptyPlace, *ResourceFields]))
        ...
        with login(Server.S10k, '0bomb1', headless=True) as sws:
            if sws:
                ...
                while find_building(sws, BuildingType.MainBuilding).level < MB_LEVEL:
                    assert level_up_building_at(sws, mb_id, True, True)
                ...
                # - Wall
                # - Cranny
                check_buildings_to_be(sws, expected_buildings)
                ...
                while find_building(sws, BuildingType.Cranny).level < CRANNY_MAX_LEVEL:
                    assert level_up_building_at(sws, cranny_index, True, True)

                # S6. Attempt to level it up again.
                # O6. Level up should fail.
                assert level_up_building_at(sws, cranny_index, True, True) == False

                # S7. Attempt to build another Cranny.
                # O7. Construction should succeed.
                assert construct_building(sws, BuildingType.Cranny, True, True)
                ...
                assert construct_building(sws, BuildingType.Stable, True, True)
                ...
                assert construct_building(sws, BuildingType.TownHall, True, True)
                ...
                assert construct_building(sws, BuildingType.Residence, True, True)
                ...
                assert construct_building(sws, BuildingType.Palace, True, True)
                # O15. Palace constructed alongside Embassy.
                assert check_buildings_to_be(sws, [BuildingType.MainBuilding, BuildingType.Wall, \
                    BuildingType.Palace, BuildingType.Embassy])
                ...
                assert construct_building(sws, BuildingType.SiegeWorkshop, True, True)
                ...
                while find_building(sws, BuildingType.Wall).level < WALL_LEVEL:
                    assert level_up_building_at(sws, wall_id, True, True)
