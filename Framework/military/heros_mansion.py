...
def name_hero(sws : SWS, newName : str):
    ...
    """
    status = False
    if sws.sendKeys(XPATH.HERO_NAME, None, waitFor=True):
        if sws.sendKeys(XPATH.HERO_NAME, newName):
            logger.success(f'In name_hero: The hero is named now "{newName}".')
            if sws.clickElement(XPATH.HERO_SAVE_NAME):
                status = True
                logger.success('In name_hero: The hero name is now saved.')
            else:
                logger.error('In name_hero: Couldn\'t push save name button.')
        else:
            logger.error('In name_hero: Couldn\'t send the new hero name in the textbox.')
    else:
        logger.error('In name_hero: Couldn\'t send the new hero name in the textbox.')
    return status
